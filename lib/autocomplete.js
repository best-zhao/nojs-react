...
var {React, ReactDOM, mixins, utils} = nj
var $ = require('jquery')
...
var Autocomplete = module.exports = React.createClass({
    ...
    filter (value) {
        ...
        var done = results => {
            this.setState({results:results.slice(0,max), index:null})
    ...
    render () {
        ...
        if( !container ){
            var list = <ul>{
                results.map((item,i)=>{
                    item = getItem(item)
                    return <li key={item} onClick={this.select.bind(this,i,'click')} className={i===index?'active':''}>{item}</li>
                })
            }</ul>
        }
        return (
