...
var Scroll = React.createClass({
    ...
    componentDidMount () {
        ...
        $(itemsComponent.refs.wrap).hover(()=>{
            this.stop();
        }, ()=>{
            this.start();
        });        
...
var ScrollItems = React.createClass({
    ...
    render () {
        ...
        var {children, _children, className} = this.props
...
var ScrollPage = React.createClass({
    ...
    render () {
        ...
        var {parentComponent, item} = this.state
        ...
        <div className="nj-scroll-page">
            {items.map((n,i)=>{
                ...
                var _item = typeof item=='function' ? <span dangerouslySetInnerHTML={{__html:item.call(this,i)}}></span> : n
                return trigger=='hover' ? 
                    <span ref={'item'+i} onMouseEnter={this.handleClick.bind(this,n-1)} className={className} key={n}>{_item}</span>
                    :<span ref={'item'+i} onClick={this.handleClick.bind(this,n-1)} className={className} key={n}>{_item}</span>
            })}
        </div>
        )
    }
